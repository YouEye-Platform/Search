...
test("Search declares the Wave 3A surface contract", () => {
  ...
  const infoHost = read("src/components/info-card-container.tsx");
  assert.ok(infoHost.includes("youeye:ready"));
  assert.ok(infoHost.includes("youeye:resize"));
  assert.doesNotMatch(infoHost, /youeye-card-ready|contentHeight/);
});

test("Search runtime source no longer emits legacy embed messages", () => {
